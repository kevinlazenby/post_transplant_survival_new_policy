...
do_data_wrangling <- function(input_tx_list = NULL,
  ...
  input_tx_list <- input_tx_list %>%
    ...
    mutate(
      ...
      # candidates at Status 1B or Status 3 who were on high-dose inotropic 
      # support or supported by multiple inotropes
      high_dose_inotropes = case_when(
        REC_INOTROP == 1 & REC_VAD_TY == 1 & 
          ((CAN_LAST_STAT == 2010 & ECMO == 0 & IABP == 0) | 
             CAN_LAST_STAT == 2130) ~ 1, 
        TRUE ~ 0),
      
      # candidates at Old Status 2 or New Status 4 who were on inotropic support
      low_dose_inotropes = case_when(
        REC_INOTROP == 1 & REC_VAD_TY == 1 &
          (CAN_LAST_STAT == 2020 | CAN_LAST_STAT == 2140) ~ 1,
        TRUE ~ 0),
...
construct_km_model <- function(input_cohort_list = NULL,
  ...
  survival_plot <- ggsurvplot(
    ...
    font.x = c(font_x, "bold"),
    font.y = c(font_y, "bold"),
    font.tickslab = c(font_ticks, "plain"),
    font.legend = c(font_legend, "bold"),
    ncensor.plot = FALSE,
